...
% function rgb = jpeg_ycbcr2rgb(ycbcr)
%     [rr, cc, ~] = size(ycbcr);
%     rgb = ycbcr;
%     % rgb = double(ycbcr);
%     ycbcr2rgb_mat = [1, 0, 1.402;
%                 1, -0.344136, -0.714136;
%                 1, 1.772, 0
%                 ]';
%     ycbcr2rgb_addmat = [-128 * 1.402, -128 * (-0.344136 - 0.714136), -128 * 1.772];

%     parfor c = 1:cc

%         for r = 1:rr
%             ycbcr_t = reshape(ycbcr(r, c, :), 1, []);
%             ycbcr_t = double(ycbcr_t);
%             rgb_t = ycbcr_t * ycbcr2rgb_mat + ycbcr2rgb_addmat;
%             rgb(r, c, :) = (rgb_t);
%             % ycbcr(r, c, :) = ycbcr_t;
%         end
...
% 注：以上是自己写的；以下是根据matlab的库修改的。由于matlab库内rgb2ycbcr出来范围16-255，需求是0-255；但是matlab这个写法运算速度非常快，所以搬过来修改转换矩阵，让输出和上面自己写的结果相等

function rgb = jpeg_ycbcr2rgb(varargin)
    ...
    %   References:
    %   ----------
    %   [1] Charles A. Poynton, "A Technical Introduction to Digital Video",
    %       John Wiley & Sons, Inc., 1996, p. 175-176
    %
    %   [2] Rec. ITU-R BT.601-5, "STUDIO ENCODING PARAMETERS OF DIGITAL TELEVISION
